...
1. **超额拒绝（503），不降级。** 日额度烧完时返回世界观内的拒绝文案，
   而不是切到本地规则桩"继续能玩"。内测的全部目的是收真实数据 ——
   悄悄换成规则桩等于往数据集里灌垃圾局，比让人今天玩不了更糟。
   （main.py 的 /api/turn 落地这条；被拒的轮次不产生任何 turn 埋点。）
2. **频率限制返回 429。** 那是异常行为（脚本刷），不是正常玩家会撞到的。
