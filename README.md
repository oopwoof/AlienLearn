...
**第七轮**（把上一轮的欠账变成套件，然后立刻还债）：上一轮的教训是
「单轮指标盖不住跨轮链路」，这一轮就把「一局能不能在 N 轮内通关」做成了
可跑的 `eval/playthrough.py`。它跑起来的当天抓到两个 bug：**秘密还没解锁
就判胜**（NPC 一句意味深长的话被判成泄密，玩家没走到最后一幕就赢了），
以及**信号提取器只读当前这一轮，而任务目标是累积的**（玩家说完正事接着
闲聊，提取器就一路判 stay）。后者不是新场景的毛病 —— 正典场景一样中招，
ramen_en 的幕推进落在第 2/11/13 轮，14 句台词打完还没通关。给提取器补上
这一幕的累积上下文之后，五个场景收敛到同一个节奏（2/4/6 推进、7-9 轮通关），
