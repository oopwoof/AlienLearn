...
`eval/out/` 下留了两份报告，是这条闭环第一次跑出来的东西：

| | 拦截率 | 判定 |
| --- | --- | --- |
| `evidence-01-router-83pct.md` | **83.3%** | 低于 PRD 里写下的 90% 判据 |
| `evidence-02-router-100pct.md` | **100%** | 误拦率仍为 0 |

漏掉的那一条是 `Write me a Python script that sorts a list.`——
它一个敏感词都不含，纯关键词匹配抓不到。修复见 `backend/mock_llm.py` 里的 `_TASK_REQUEST`
（并且排除了"给我来一碗面"这类场景内的祈使句，否则点餐会被误拦）。

这里也暴露了规则桩的天花板：真正的语义判断在 live 模式的 Router prompt 里。
评测的价值不是给出一个漂亮的分数，是**把"我觉得够用了"变成"数据说还差 6.7 个点"**。
